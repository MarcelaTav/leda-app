...
function usarListaPersistida(chave, userId) {
  ...
  useEffect(() => {
    ...
    // Carregamento com novas tentativas. Dois perigos são tratados aqui:
    // 1) a consulta falhar (rede oscilando) — tenta de novo;
    // 2) a consulta "dar certo" e voltar vazia porque a sessão de login ainda
    //    estava sendo renovada — nesse caso o banco não reconhece quem pergunta
    //    e responde "nenhum registro", sem erro. Por isso resposta vazia é
    //    reconfirmada algumas vezes antes de ser aceita como verdade.
    // Enquanto não houver leitura confiável, "loaded" continua falso: a tela
    // segue mostrando "Carregando…" e a gravação fica bloqueada.
    const CONFIRMACOES_VAZIO = 2;

    async function tentarCarregar(tentativa, vaziosSeguidos) {
      try {
        // garante que a sessão está de fato válida antes de perguntar ao banco
        const { data: sessaoAtual } = await supabase.auth.getSession();
        if (cancelado) return;
        if (!sessaoAtual || !sessaoAtual.session) {
          throw new Error("sessao ainda nao disponivel");
        }
        ...
        // veio vazio: pode ser primeira vez usando (correto) ou sessão que
        // ainda não valeu (falso vazio). Confirma antes de aceitar.
        if (vaziosSeguidos + 1 >= CONFIRMACOES_VAZIO) {
          setLoaded(true);
          return;
        }
        timer = setTimeout(() => {
          if (!cancelado) tentarCarregar(tentativa, vaziosSeguidos + 1);
        }, 600);
      } catch (e) {
        if (cancelado) return;
        const espera = Math.min(800 * Math.pow(2, tentativa), 8000);
        timer = setTimeout(() => {
          if (!cancelado) tentarCarregar(tentativa + 1, vaziosSeguidos);
        }, espera);
      }
    }
  ...
  function salvar(novaLista) {
    // trava de segurança: enquanto os dados reais não tiverem sido lidos do
    // banco, gravar significaria apagar o que existe lá. Nesse caso a lista
    // ainda aparece como "Carregando…", então não há edição legítima em curso.
...
function usarValorPersistido(chave, valorInicial, userId) {
  ...
  useEffect(() => {
    ...
    // mesma proteção do hook de lista, incluindo a reconfirmação de resposta
    // vazia (que pode significar apenas que a sessão ainda não valia)
    const CONFIRMACOES_VAZIO = 2;
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
          ...
          <div
            style={{
              flex: 1,
              minHeight: 0,
              overflowY: modo === "leitura" ? "auto" : "hidden",
              overflowX: "hidden",
            ...
            {modo === "leitura" ? (
              <>
                ...
                {resenhaSelecionada.sobreAutor && (
                  <>
                    <div style={{ flexShrink: 0, fontFamily: SANS, fontSize: "10.5px", letterSpacing: "0.1em", textTransform: "uppercase", color: COR.melEscuro, marginBottom: "7px" }}>Sobre o autor</div>
                    <div
                      style={{
                        flexShrink: 0,
                        fontSize: "14px",
                ...
                <div style={{ flexShrink: 0, fontSize: "11.5px", color: COR.textoSecundario, marginBottom: "16px" }}>
                  Atualizada em {formatarData(resenhaSelecionada.atualizadoEm)}
                </div>
              </>
            ) : (
              <form onSubmit={salvarResenha} style={{ display: "flex", flexDirection: "column", gap: "10px", flex: 1, minHeight: 0 }}>
                <div style={{ flexShrink: 0 }}>
                ...
                <div style={{ flex: 1.5, minHeight: "90px", marginTop: "2px", display: "flex", flexDirection: "column" }}>
                  ...
                  <textarea
                    ...
                    style={{ width: "100%", boxSizing: "border-box", flex: 1, minHeight: 0, overflowY: "auto", overflowX: "hidden", resize: "none", fontSize: "16px", lineHeight: 1.6, padding: "14px 15px", borderRadius: "12px", border: `1.5px solid ${COR.mel}66`, background: "#FFFEFB", boxShadow: "inset 0 1px 3px rgba(62, 58, 49, 0.05)", fontFamily: "inherit", color: COR.textoPrincipal, outline: "none" }}
                  />
                </div>
                ...
                <div onClick={() => setPublica(!publica)} style={{ flexShrink: 0, marginTop: "6px", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", borderRadius: "10px", background: publica ? COR.melClaro : COR.saugeClaro, cursor: "pointer" }}>
                  <div>
                ...
                {erroForm && <div style={{ flexShrink: 0, fontSize: "12.5px", color: COR.alerta }}>{erroForm}</div>}

                <div style={{ flexShrink: 0, display: "flex", gap: "10px", marginTop: "14px", marginBottom: "2px" }}>
...
function TelaEscritos({ estado, pendente, corPersonalizada, onMudarCor, marcadoresState }) {
  ...
  if (vista === "leituraCapitulo" && capituloSelecionado) {
    return (
      <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
        <div style={{ flexShrink: 0, padding: "16px 20px 0", borderBottom: `1px solid ${COR.linha}` }}>
          <button
            onClick={() => setVista("lista")}
            style={{ fontFamily: SANS, fontSize: "12.5px", color: COR.textoSecundario, background: "transparent", border: "none", padding: 0, cursor: "pointer" }}
          >
          ...
          {/* faixa fixa: identidade da obra sempre visível durante a leitura */}
          <div
            style={{
              display: "flex",
              alignItems: "center",
              ...
              padding: "10px 0 12px",
              marginTop: "10px",
              borderTop: `1px solid ${corBase}22`,
            }}
          >
            <span style={{ width: "18px", height: "1px", background: `${corBase}66`, flexShrink: 0 }} />
            <span style={{ fontFamily: SERIF, fontSize: "14.5px", fontWeight: 700, color: COR.textoPrincipal, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
              {obraSelecionada.titulo}
            </span>
            {obraSelecionada.genero && (
              <span style={{ flexShrink: 0, fontFamily: SANS, fontSize: "9.5px", fontWeight: 600, letterSpacing: "0.04em", padding: "2px 10px", borderRadius: "20px", border: `1px solid ${corBase}`, color: corEscura }}>
                {obraSelecionada.genero}
              </span>
            )}
            <span style={{ width: "18px", height: "1px", background: `${corBase}66`, flexShrink: 0 }} />
        ...
        <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", overscrollBehavior: "contain", WebkitOverflowScrolling: "touch", background: `linear-gradient(180deg, ${corClara}55, ${COR.fundo} 140px)` }}>
          <div style={{ maxWidth: "560px", margin: "0 auto", padding: "20px 20px 40px" }}>
            {(obraSelecionada.sinopse || (obraSelecionada.marcadorIds || []).length > 0) && (
                ...
                {obraSelecionada.sinopse && (
                  <div style={{ position: "relative", padding: "6px 10px 0" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", marginTop: "18px" }}>
                      <span style={{ width: "34px", height: "1px", background: `${corBase}40` }} />
                      <span style={{ width: "4px", height: "4px", borderRadius: "50%", background: `${corBase}70` }} />
                      <span style={{ width: "34px", height: "1px", background: `${corBase}40` }} />
                    </div>
                  </div>
                )}
                {(obraSelecionada.marcadorIds || []).length > 0 && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", justifyContent: "center", marginTop: obraSelecionada.sinopse ? "16px" : 0 }}>
                    {(obraSelecionada.marcadorIds || []).map((mid) => {
                      const m = marcadores.find((mm) => mm.id === mid);
                      if (!m) return null;
                      return (
